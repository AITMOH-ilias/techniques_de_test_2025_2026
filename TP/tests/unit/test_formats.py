...
class TestPointSetFormats:
    ...
    def test_lecture_octets_ok(self):
        ...
        assert point_set.points[0] == (0.0, 0.0)
        assert point_set.points[1] == (1.0, 2.0)
    ...
    def test_endianness_stricte(self):

        # 1. Test Entier (Unsigned Long 4 bytes)
        valeur_int = 1
        packed_int = struct.pack('>I', valeur_int)
        assert packed_int == b'\x00\x00\x00\x01', "L'entier doit être encodé en Big-Endian"

        # 2. Test Flottant (Float 4 bytes - IEEE 754)
        valeur_float = 1.0
        packed_float = struct.pack('>f', valeur_float)
        assert packed_float == b'\x3f\x80\x00\x00', "Le flottant doit être encodé en Big-Endian"
